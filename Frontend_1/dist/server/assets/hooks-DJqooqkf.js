...
api.interceptors.request.use((config) => {
	...
	if (WRITE_API_KEY && config.method && config.method.toLowerCase() !== "get") config.headers["X-API-Key"] = WRITE_API_KEY;
	return config;
});
...
async function downloadCsv(path, filename) {
	const response = await api.get(normalizePath(path), { responseType: "blob" });
...
function useApiQuery(key, path, params, refetchInterval) {
	return useQuery({
...
var useDiscoveryRuns = () => useApiQuery(["discovery-runs"], "/discovery/runs", void 0, 2e3);
